...
--[[
    ...
    + size = Tamanho na tela ex. 100vh tamanho original
    + Callback = Define se o minigame deu sucesso ou falhou
        'sucess' or 'failed'

    TriggerEvent("mxStartLockpickGame", x, y, scale, size, function(Callback)
...
RegisterCommand('startlockpick', function(src, args, cmd) 
    TriggerEvent("mxStartLockpickGame", args[1], args[2], args[3], args[4], function(Callback)
        if Callback == 'sucess' then
            print(">> Lockpick: [ Success ]")
        elseif Callback == 'failed' then
            print(">> Lockpick: [ Failed ]")
...
AddEventHandler('mxStartLockpickGame', function(x, y, scale, size, Callback)
    LockpickGame(x, y, scale, size, Callback)
end)

function LockpickGame(x, y, scale, size, Callback)
